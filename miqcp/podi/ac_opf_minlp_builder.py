...
TAN_SUBST_VARIABLE_NAME = 'tan_subst'
TAN_DELTA_VARIABLE_NAME = 'tan_delta'
TAN_ERROR_VARIABLE_NAME = 'tan_error'
...
class AcOpfMinlpBuilder(object):
    ...
    def build_ac_opf_variables(self, ac_opf_minlp, network, parameters, curtailment_info):    
        ...
        self.build_ac_opf_variables_tan_subst(ac_opf_minlp, network, parameters)
    ...
    def build_ac_opf_constraints(self, ac_opf_minlp, network, parameters, curtailment_info):
        ...
        self.build_ac_opf_constraints_tan_subs(ac_opf_minlp, network, parameters)
    ...
    def build_ac_opf_variables_voltage_square(self, ac_opf_minlp, network, parameters):
        ...
        for node_id in network.graph.nodes:    #for every bus
            ...
            voltage_square_lower_bound = bus_data['minimum_voltage_magnitude']**2    #read lower bound
    ...
    def build_ac_opf_variables_tan_subst(self, ac_opf_minlp, network, parameters):
        tan_subst = {}
        tan_delta = {}
        tan_error = {}
        for (from_bus_id, to_bus_id) in network.graph.edges:
            angle_var = ac_opf_minlp.variables[PHASE_ANGLE_DIFFERENCE_VARIABLE_NAME][from_bus_id, to_bus_id]
       
            [lb,ub] = find_1d_var_bounds('tan', angle_var.lower_bound, angle_var.upper_bound)
            tan_subst_var = Variable(TAN_SUBST_VARIABLE_NAME + '_' + str(from_bus_id) + '_' + str(to_bus_id), 'continuous', lb, ub)
            tan_subst_var.set_given_initial_value( tan(angle_var.given_initial_value) )
            tan_subst.update( { (from_bus_id, to_bus_id) : tan_subst_var } )
       
            tan_delta_var = Variable(TAN_DELTA_VARIABLE_NAME + '_' + str(from_bus_id) + '_' + str(to_bus_id), 'continuous', 0, 1)
            tan_delta_var.set_given_initial_value( (angle_var.given_initial_value-angle_var.lower_bound)/(angle_var.upper_bound-angle_var.lower_bound) )
            tan_delta.update( { (from_bus_id, to_bus_id) : tan_delta_var } )
            
            [lower_error, upper_error] = compute_error_on_one_1d_segment('tan', angle_var.lower_bound, angle_var.upper_bound)
            tan_error_var = Variable(TAN_ERROR_VARIABLE_NAME + '_' + str(from_bus_id) + '_' + str(to_bus_id), 'continuous', -upper_error, lower_error)
            tan_error_var.set_given_initial_value( 0 )
            tan_error.update( { (from_bus_id, to_bus_id) : tan_error_var } )
         
        ac_opf_minlp.add_variable({TAN_SUBST_VARIABLE_NAME : tan_subst})
        ac_opf_minlp.add_variable({TAN_DELTA_VARIABLE_NAME : tan_delta})
        ac_opf_minlp.add_variable({TAN_ERROR_VARIABLE_NAME : tan_error})
    ...
    def build_ac_opf_constraints_tan_subs(self, ac_opf_minlp, network, parameters):
        x_value = {}
        y_value = {}
        
        for inner_key in ac_opf_minlp.variables[TAN_SUBST_VARIABLE_NAME].keys():
        
            angle_var = ac_opf_minlp.variables[PHASE_ANGLE_DIFFERENCE_VARIABLE_NAME][inner_key]
            subst_var = ac_opf_minlp.variables[TAN_SUBST_VARIABLE_NAME][inner_key]
            delta_var = ac_opf_minlp.variables[TAN_DELTA_VARIABLE_NAME][inner_key]
            error_var = ac_opf_minlp.variables[TAN_ERROR_VARIABLE_NAME][inner_key]
            
            x_value_constraint = LinearConstraint( 'tan_subst_x_value_' + str(inner_key))
            vertices = [angle_var.lower_bound, angle_var.upper_bound]
            expression = Eq(angle_var.sympy_symbol , vertices[0] + (vertices[1] - vertices[0]) * delta_var.sympy_symbol  ) 
            variables = [angle_var, delta_var]
            x_value_constraint.build_linear_constraint_expression_direct(expression, variables)
            x_value.update( {inner_key : x_value_constraint} )
            
            y_value_constraint = LinearConstraint( 'tan_subst_y_value_'+str(inner_key) )
            expression = Eq(subst_var.sympy_symbol , tan(vertices[0]) + (tan(vertices[1]) - tan(vertices[0])) * delta_var.sympy_symbol  + error_var.sympy_symbol )
            variables = [angle_var, delta_var, error_var]
            y_value_constraint.build_linear_constraint_expression_direct(expression, variables)
            y_value.update( {inner_key : y_value_constraint} )
            
            
        ac_opf_minlp.add_constraint( { 'tan_subst_x_value' : x_value } )
        ac_opf_minlp.add_constraint( { 'tan_subst_y_value' : y_value } )        
    ...
    if number == '':
        ...
        if TRANSMISSION_SWITCH_VARIABLE_NAME in minlp.variables.keys():    
            if parameters['univariate_approximation'] == 1:
                ...
                expression_left_side = variables[0].sympy_symbol**2  + variables[1].sympy_symbol**2 
...
def build_trigonometric_constraint(constraint_name, from_bus_id, to_bus_id, minlp, number='', parameters=''):
    if -TAN_SWITCHING_ANGLE < minlp.variables[PHASE_ANGLE_DIFFERENCE_VARIABLE_NAME][from_bus_id, to_bus_id].lower_bound and minlp.variables[PHASE_ANGLE_DIFFERENCE_VARIABLE_NAME][from_bus_id, to_bus_id].lower_bound< TAN_SWITCHING_ANGLE:
        ...
        if number == '':
            constraint = NonlinearConstraint(constraint_name + '_' + str(from_bus_id) + '_' + str(to_bus_id))
            if parameters['univariate_approximation'] == 1:
                variables = [ minlp.variables[VOLTAGE_PRODUCT_REAL_VARIABLE_NAME][from_bus_id,to_bus_id], minlp.variables[TAN_SUBST_VARIABLE_NAME][from_bus_id,to_bus_id], minlp.variables[VOLTAGE_PRODUCT_IMAGINARY_VARIABLE_NAME][from_bus_id,to_bus_id]  ]
                expression_left_side =  ( variables[0].sympy_symbol  - variables[1].sympy_symbol )**2 - variables[0].sympy_symbol**2 -  variables[1].sympy_symbol**2
                expression_right_side = 2 * variables[2].sympy_symbol
            else:
                variables = [ minlp.variables[VOLTAGE_PRODUCT_REAL_VARIABLE_NAME][from_bus_id,to_bus_id], minlp.variables[TAN_SUBST_VARIABLE_NAME][from_bus_id,to_bus_id], minlp.variables[VOLTAGE_PRODUCT_IMAGINARY_VARIABLE_NAME][from_bus_id,to_bus_id]  ]
                expression_left_side = - variables[0].sympy_symbol * variables[1].sympy_symbol
                expression_right_side = variables[2].sympy_symbol
        
        else:
            constraint = NonlinearConstraint(constraint_name + '_' + str(from_bus_id) + '_' + str(to_bus_id) + '_' + str(number))
            if parameters['univariate_approximation'] == 1:
                variables = [ minlp.variables[VOLTAGE_PRODUCT_REAL_VARIABLE_NAME][from_bus_id,to_bus_id,number], minlp.variables[TAN_SUBST_VARIABLE_NAME][from_bus_id,to_bus_id], minlp.variables[VOLTAGE_PRODUCT_IMAGINARY_VARIABLE_NAME][from_bus_id,to_bus_id,number]  ]
                expression_left_side =  ( variables[0].sympy_symbol - variables[1].sympy_symbol )**2 - variables[0].sympy_symbol**2 - variables[1].sympy_symbol**2
                expression_right_side = 2 * variables[2].sympy_symbol
            else:
                variables = [ minlp.variables[VOLTAGE_PRODUCT_REAL_VARIABLE_NAME][from_bus_id,to_bus_id,number], minlp.variables[TAN_SUBST_VARIABLE_NAME][from_bus_id,to_bus_id], minlp.variables[VOLTAGE_PRODUCT_IMAGINARY_VARIABLE_NAME][from_bus_id,to_bus_id,number]  ]
...
def build_reactive_power_flow_equation(constraint_name, from_bus_id, to_bus_id, minlp, network, number=''):
    ...
    #ratio = 1    #Generator ratio end of line
    if ratio == 0:
        ratio = 1  
