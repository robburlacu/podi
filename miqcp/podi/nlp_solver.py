        ...
        if ss==1 and (ms == 1 or ms ==2):    #if local solution was found
            ...
            if found_better_solution == 1:
                for outer_key in mip.variables.keys():
                    if len(str(outer_key).split(SUBSTITUTION_VARIABLES_SUFFIX)) == 1 and len(str(outer_key).split(RELAXATION_SUFFIX)) == 1 and len(str(outer_key).split('cuts')) == 1:  
