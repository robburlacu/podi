...
from podi.ac_opf_minlp_builder import GENERATOR_SWITCH_VARIABLE_NAME, TRANSMISSION_SWITCH_VARIABLE_NAME
...
class Mipsolver(object):
    ...
    def build_mip_for_solver(self, mip, parameters, use_penalty=0, use_pool=0, no_good_cuts=0, use_objective_cuts=1):
        
        return self.build_gurobi_mip(mip, parameters, use_penalty, use_pool, no_good_cuts, use_objective_cuts)
            
            
    def build_gurobi_mip(self, mip, parameters, use_penalty, use_pool, no_good_cuts, use_objective_cuts=1):
        gurobi_mip = self.initialize_gurobi_mip(mip)
        self.set_gurobi_parameters(gurobi_mip, parameters, mip, use_pool)
        self.add_gurobi_vars(gurobi_mip, mip, parameters)
        ...
        if no_good_cuts == 1:
            self.add_no_good_cuts(gurobi_mip, mip, parameters)
    ...
    def add_gurobi_vars(self, gurobi_mip, mip, parameters):
                ...
                if outer_key.split('_')[-1] == RELAXATION_SUFFIX:
                    for relaxation_key in mip.variables[outer_key][inner_key].keys():
                        if relaxation_key == DELTA_METHOD_ERROR_KEY:
                            ...
                            elif parameters['using_mip_start'] == 1 and mip.objective.orientation == 'max' and mip.lower_obj_bound > -inf:
                                var.start = mip.variables[outer_key][inner_key][relaxation_key].solution_value
                        else:
    ...
    def add_no_good_cuts(self, gurobi_mip, mip, parameters):
    ...
    def solve_and_save_results(self, gurobi_mip, mip, callback='', use_penalty=0, no_good_cuts=0, time_limit=inf):
    ...
    def check_stopping_criteria(self, mip, parameters, time_start, log_file):
        flag = False
