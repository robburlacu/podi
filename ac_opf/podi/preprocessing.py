...
class Preprocessor(object):
    ...
    def preprocessing(self, mip, mip_for_solver, parameters, log_file, big_log_file, extra=''):
        if parameters['feasibility_based_bound_tightening'] == 1:
            self.feasibility_based_bound_tightening(mip, mip_for_solver, log_file, big_log_file)
        if parameters['bound_tightening_using_mip'] == 1:
            self.bound_tightening_using_mip(mip, mip_for_solver, parameters, log_file, big_log_file, extra)
        if parameters['feasibility_based_bound_tightening'] == 1:
            self.feasibility_based_bound_tightening(mip, mip_for_solver, log_file, big_log_file)


    def feasibility_based_bound_tightening(self, mip, mip_for_solver, log_file, big_log_file):
        counter = 0
        start_fbbt = time()
            ...
            for inner_key in mip.variables[var_name].keys():
                ...
                if lower_bound > var.lower_bound:
                    ...
                    if abs(lower_bound-var.upper_bound) < MIN_RANGE_FOR_BOUND_TIGHTENING:    #avoid numerical issues
                        new_bound = floor(max([lower_bound - MIN_RANGE_FOR_BOUND_TIGHTENING, var.lower_bound])*(10**NUMBER_DIGITS))/(10**NUMBER_DIGITS)
                        old_bound = var.lower_bound
                        var.set_variable_lower_bound( new_bound )
                    else:
                        new_bound = floor(lower_bound*(10**NUMBER_DIGITS))/(10**NUMBER_DIGITS)
                        old_bound = var.lower_bound
                        var.set_variable_lower_bound( new_bound )
                if upper_bound < var.upper_bound:
                    counter = counter + 1
                    old_bound = var.upper_bound
                    if abs(upper_bound-var.lower_bound) < MIN_RANGE_FOR_BOUND_TIGHTENING:
                        new_bound = ceil(min([upper_bound+MIN_RANGE_FOR_BOUND_TIGHTENING, var.upper_bound])*(10**NUMBER_DIGITS))/(10**NUMBER_DIGITS)
                        var.set_variable_upper_bound(new_bound)
                    else:
                        new_bound = ceil(upper_bound*(10**NUMBER_DIGITS))/(10**NUMBER_DIGITS)
                        var.set_variable_upper_bound( new_bound )

        print('FBBT improved', counter, 'Bounds', ' in', time() - start_fbbt, 's')
    ...
    def bound_tightening_using_mip(self, mip, mip_for_solver, parameters, log_file, big_log_file, extra=''):
        start_obbt = time()
        ...
        if parameters['mip_solver'] == 'Gurobi':
            ...
            mip_for_solver.Params.NodeLimit = parameters['max_nodes_bound_tightening_using_mip'+extra]
            mip_for_solver.Params.OutputFlag = 0
            mip_for_solver.Params.TimeLimit = parameters['max_time_bound_tightening_using_mip'+extra]

            mip_for_solver.Params.FeasibilityTol = 0.01*parameters['FeasibilityTol']
            mip_for_solver.Params.MIPGap = parameters['opt_gap_bound_tightening_using_mip'+extra]
            ...
            for var_name in var_names:
                ...
                max_improvement_per_var_abs = 0.0
                max_improvement_per_var_rel = 0.0
                counter_per_var = 0
                ...
                for inner_key in mip.variables[var_name].keys():
                    var = mip.variables[var_name][inner_key]
                    length_dic[ inner_key] = var.upper_bound - var.lower_bound
                
                for inner_key in dict(sorted(length_dic.items(), key=lambda item: -item[1])).keys():
                    ...
                    try:
                        ...
                        if abs(mip_for_solver.ConstrVio) > parameters['FeasibilityTol']:    #using convex MINLP: Gurobi might ignore feasibilityTol
                    ...
                    if mip_var.lower_bound + NUM_TOL < mip_for_solver.ObjBound and ignore == 0:
                        ...
                        if lower_bound > mip_var.lower_bound:
                            old_bound = mip_var.lower_bound
                            new_bound = floor(lower_bound*(10**NUMBER_DIGITS))/(10**NUMBER_DIGITS)
                            
                            improvement_abs[i] = abs(mip_var.lower_bound - new_bound) 
                            if max(abs(mip_var.lower_bound),NUM_TOL) == NUM_TOL:
                                improvement_rel[i] = 0
                            else:
                                improvement_rel[i] = abs(mip_var.lower_bound - new_bound)/max(abs(mip_var.lower_bound),NUM_TOL)
                            
                            max_improvement_per_var_abs = max_improvement_per_var_abs + improvement_abs[i]
                            max_improvement_per_var_rel = max_improvement_per_var_rel + improvement_rel[i]
                            
                            mip_var.set_variable_lower_bound( lower_bound )

                            mip_for_solver.getVarByName(mip_var.id).setAttr('lb', new_bound )
                            counter = counter + 1
                    ...
                    mip_for_solver.update()
                    #mip_for_solver.Params.OutputFlag = 1
                    mip_for_solver.optimize()
                    ...
                    if mip_var.upper_bound > mip_for_solver.ObjBound + NUM_TOL and ignore == 0: #and abs(mip_for_solver.ConstrVio) <= parameters['FeasibilityTol']:
                        ...
                        if upper_bound < mip_var.upper_bound:
                            old_bound = mip_var.upper_bound
                            new_bound = ceil(upper_bound*(10**NUMBER_DIGITS))/(10**NUMBER_DIGITS)
                            
                            improvement_abs[i] = abs(mip_var.upper_bound - new_bound) 
                            if max(abs(mip_var.upper_bound),NUM_TOL) == NUM_TOL:
                                improvement_rel[i] = 0
                            else:
                                improvement_rel[i] = abs(mip_var.upper_bound - new_bound)/max(abs(mip_var.upper_bound),NUM_TOL)   
    
                            max_improvement_per_var_abs = max_improvement_per_var_abs + improvement_abs[i]
                            max_improvement_per_var_rel = max_improvement_per_var_rel + improvement_rel[i]
                            
                            mip_var.set_variable_upper_bound( upper_bound)
         
                            mip_for_solver.getVarByName(mip_var.id).setAttr('ub', new_bound)
                            counter = counter + 1
                            counter_per_var = counter_per_var + 1
                    ...
                    i=i+1
                print('OBBT for', var_name, 'improved', counter_per_var, 'with average improvement of ', max_improvement_per_var_abs/max(1,counter_per_var), '(',max_improvement_per_var_rel/max(1,counter_per_var)*100,'%)')                                         
                log_file.write('OBBT for ' + str(var_name) + ' improved '+ str(counter_per_var) + ' with average improvement of ' + str(max_improvement_per_var_abs/max(1,counter_per_var)) + ' (' + str(max_improvement_per_var_rel/max(1,counter_per_var)*100) + '%)\n')
