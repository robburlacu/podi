...
#            print('DOMAIN REDUCTION POSSIBLE DUE TO SYMMETRY', self.id, 'with', reduction,'(',100*reduction/(var1.upper_bound-var1.lower_bound),'%)')
        if flag == 1 and flag1 == 0 and flag2 == 1:
            reduction = min(abs(var2.lower_bound),abs(var2.upper_bound))
#            print('DOMAIN REDUCTION POSSIBLE DUE TO SYMMETRY', self.id, 'with', reduction,'(',100*reduction/(var2.upper_bound-var2.lower_bound),'%)')
...
#            print('DOMAIN REDUCTION POSSIBLE DUE TO SYMMETRY', self.id, 'with', reduction,'(',100*reduction/((var1.upper_bound-var1.lower_bound)*(var2.upper_bound-var2.lower_bound)),'%)')
    ...
    def test_if_domain_reduction_would_reduce_binary_symmetry_1d(self):
        ...
        if flag == 1:
            ...
            #print('DOMAIN REDUCTION POSSIBLE DUE TO SYMMETRY', self.id, 'with', reduction,'(',100*reduction/(var.upper_bound-var.lower_bound),'%)')
        return flag
        
    def change_to_domain_reduction(self, abs_var):
        vertices= self.triangulation.vertices
        reduction = min(abs(min(vertices)),abs(max(vertices)))
        #print('DOMAIN REDUCED DUE TO SYMMETRY', self.id, 'with', reduction,'(',100*reduction/(max(vertices)-min(vertices)),'%)')
