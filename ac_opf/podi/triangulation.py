...
class Triangulation1d(object):
    ...
    def create_triangulation_from_boundaries(self, bounds, number_of_desired_segments):
        ...
        if bounds[1]-bounds[0] == 0:
            #log_info('Triangulation Domain invalid ' + str(self.id))
    ...
    def enforce_minimum_segment_length(self, length):
        for i in range(self.number_of_segments):
            point1 = self.vertices[i]
            point2 = self.vertices[i+1]
            true_leng = point2 - point1
            if true_leng > length:
                number_of_new_points = ceil(true_leng/length) - 1
                for point_i in range(1, number_of_new_points+1):
                    new_point = point1 + point_i * true_leng/(number_of_new_points+1)
    ...
    def tight_vertices_with_new_bounds(self, lb, ub):
        new_vertices = []
        new_vertices.insert(0 , lb)
        new_vertices.insert(1, ub)
        for point in self.vertices:
            if point > lb and point < ub:
                new_vertices.insert( len(new_vertices)-1, point)
        self.vertices = new_vertices
        self.number_of_segments = len(new_vertices)-1
