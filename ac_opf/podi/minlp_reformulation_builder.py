...
#import numpy as np
from sympy import Eq, postorder_traversal, sin, cos, tan, pi
...
class MinlpReformulationBuilder(object):
    ...
    def build(self, minlp):        
        minlp_reformulation = Minlp( minlp.id.replace(MINLP_ID_PREFIX, '') + '_' + REFORMULATION_SUFFIX )  
        self.build_minlp_reformulation(minlp_reformulation, minlp)   
        return minlp_reformulation

    
    
    def build_minlp_reformulation(self, minlp_reformulation, minlp):
        minlp_reformulation.add_variable(minlp.variables)
        new_vars_array = []
        new_constr_array = []
    ...
    def build_objective_reformulation(self, minlp_reformulation, minlp, new_vars_array, new_constr_array):
        if minlp.objective.type == 'linear':
            minlp_reformulation.set_objective(minlp.objective)   
...
def find_var_from_sympy_symbol(symbol, minlp, dic_new_variables, new_vars_array):
    #maybe also for substition variables TO DO
    ...
    string_part = str(symbol).split('_')    
