...
class GamsNlpWriter(object):
    ...
    def write_nlp_for_first_iteration(self, minlp, mip, parameters):
        ...
        if parameters['number_of_random_starting_values'] >= 1 and (mip.changed_binary_decission == 'yes' or mip.iteration_number == 1):
            for random_i in range(int(parameters['number_of_random_starting_values'])):
                filename5 = create_nlp_filename( mip.id + '_random_initial_'+str(random_i) )
                
                file_path = 'gams_files/' + filename5
                if  os.path.exists(file_path) and WRITE_EVERY_NLP==1:
                    new_file_name5 = str(filename4).split('.gms')[0] + '_' + str(mip.number_of_solved_nlps+1) + '.gms'
                    copyfile('gams_files/' + filename5,'gams_files/' + new_file_name5)
    
                file5 = open( 'gams_files/' + filename5, 'w' )
                file5.write( '$title "' + filename5.replace('.gms','') + '"\n')
                self.write_variable_definition_random_initial_value_first_iteration(file5, minlp, mip)
                self.write_constraints(file5, minlp, mip)
                self.write_model_definition(file5, minlp, mip)
                self.write_model_options(file5, minlp, mip)
                file5.close()  
    ...
    def write_nlp_from_relaxation_solution(self, minlp, mip, parameters):
        ...
        if parameters['number_of_random_starting_values'] >= 1 and (mip.changed_binary_decission == 'yes' or mip.iteration_number == 1):
            for random_i in range(int(parameters['number_of_random_starting_values'])):
                filename4 = create_nlp_filename( mip.id + '_random_initial_'+str(random_i) )
                ...
                file4 = open( 'gams_files/' + filename4, 'w' )
                file4.write( '$title "' + filename4.replace('.gms','') + '"\n')
                self.write_variable_definition_random_initial_value(file4, minlp, mip)
                self.write_constraints(file4, minlp, mip)
                self.write_model_definition(file4, minlp, mip)
                self.write_model_options(file4, minlp, mip)
                file4.close()
        ...
        if parameters['number_of_extra_nlps'] >= 1 and (mip.changed_binary_decission == 'yes' or mip.iteration_number == 1):
            for extra_i in range(ceil(int(parameters['number_of_extra_nlps'])/2)):
                filename4 = create_nlp_filename( mip.id + '_extra_down_initial_'+str(extra_i) )
                ...
                file4 = open( 'gams_files/' + filename4, 'w' )
                file4.write( '$title "' + filename4.replace('.gms','') + '"\n')
                self.write_variable_definition_extra_down_initial_value(file4, minlp, mip, extra_i, parameters['relative_stepzise_for_extra_nlps'])
                self.write_constraints(file4, minlp, mip)
                self.write_model_definition(file4, minlp, mip)
                self.write_model_options(file4, minlp, mip)
                file4.close()
 
                filename4 = create_nlp_filename( mip.id + '_extra_up_initial_'+str(extra_i) )
                ...
                file4 = open( 'gams_files/' + filename4, 'w' )
                file4.write( '$title "' + filename4.replace('.gms','') + '"\n')
                self.write_variable_definition_extra_up_initial_value(file4, minlp, mip, extra_i, parameters['relative_stepzise_for_extra_nlps'])
                self.write_constraints(file4, minlp, mip)
                self.write_model_definition(file4, minlp, mip)
                self.write_model_options(file4, minlp, mip)
                file4.close()
    ...
    def write_variable_definition_random_initial_value(self, file, minlp, mip):
        file.write( '\n*===== SECTION: VARIABLE DEFINITION\n' )
        file.write( 'Free Variable objval ;\n' )
        for outer_key in minlp.variables.keys():
            for inner_key in minlp.variables[outer_key].keys():
                ...
                if abs(var.lower_bound - var.upper_bound) < 1e-6:
                    ...
                    if var.variable_type == 'continuous':
                        random_number = np.random.uniform(var.lower_bound, var.upper_bound)
                        file.write( 'Free Variable ' + str(var.sympy_symbol) + ' ;\n' )
                        file.write( '\t' + str(var.sympy_symbol) + '.lo = ' + str(round(var.initial_lower_bound,DIGIT_NUMBER)) + ' ;\n' )    #lower_bound
                        file.write( '\t' + str(var.sympy_symbol) + '.up = ' + str(round(var.initial_upper_bound,DIGIT_NUMBER)) + ' ;\n')    #upper_bound
                        file.write( '\t' + str(var.sympy_symbol) + '.l = ' + str( round(random_number,DIGIT_NUMBER) ) + '  ;\n')    #initial value
                    elif var.variable_type == 'binary':
                        file.write( 'Scalar ' + str(var.sympy_symbol) + ' / ' + str( round(dc_var.relaxation_value) ) + ' / ;\n')


    def write_variable_definition_extra_down_initial_value(self, file, minlp, mip, extra_i, rel_stepsize):
        file.write( '\n*===== SECTION: VARIABLE DEFINITION\n' )
        file.write( 'Free Variable objval ;\n' )
        for outer_key in minlp.variables.keys():
            for inner_key in minlp.variables[outer_key].keys():
                ...
                if outer_key in mip.variables.keys():
                    if inner_key in mip.variables[outer_key].keys():
                        mip_var = mip.variables[outer_key][inner_key]
                if abs(var.lower_bound - var.upper_bound) < 1e-6:
                    file.write( 'Scalar ' + str(var.sympy_symbol) + ' / ' + str( round(var.lower_bound ,DIGIT_NUMBER) ) + ' / ;\n')
                else:
                    if var.variable_type == 'continuous':
                        extra_value = mip_var.relaxation_value - (extra_i+1) * rel_stepsize * abs(var.upper_bound - var.lower_bound)
                        extra_value = min([extra_value, var.upper_bound])
                        extra_value = max([extra_value, var.lower_bound])
                        file.write( 'Free Variable ' + str(var.sympy_symbol) + ' ;\n' )
                        file.write( '\t' + str(var.sympy_symbol) + '.lo = ' + str(round(var.initial_lower_bound,DIGIT_NUMBER)) + ' ;\n' )    #lower_bound
                        file.write( '\t' + str(var.sympy_symbol) + '.up = ' + str(round(var.initial_upper_bound,DIGIT_NUMBER)) + ' ;\n')    #upper_bound
                        file.write( '\t' + str(var.sympy_symbol) + '.l = ' + str( round(extra_value,DIGIT_NUMBER) ) + '  ;\n')    #initial value
                    elif var.variable_type == 'binary':
                        file.write( 'Scalar ' + str(var.sympy_symbol) + ' / ' + str( round(mip_var.relaxation_value) ) + ' / ;\n')


    def write_variable_definition_extra_up_initial_value(self, file, minlp, mip, extra_i, rel_stepsize):
        file.write( '\n*===== SECTION: VARIABLE DEFINITION\n' )
        file.write( 'Free Variable objval ;\n' )
        for outer_key in minlp.variables.keys():
            for inner_key in minlp.variables[outer_key].keys():
    ...
    def write_variable_definition_random_initial_value_first_iteration(self, file, minlp, mip):
        file.write( '\n*===== SECTION: VARIABLE DEFINITION\n' )
        file.write( 'Free Variable objval ;\n' )
        for outer_key in minlp.variables.keys():
            for inner_key in minlp.variables[outer_key].keys():
    ...
    def write_nlp_from_tight_relaxation_solution(self, minlp, mip, parameters):
        ...
        if parameters['number_of_random_starting_values'] >= 1 and (mip.changed_binary_decission == 'yes' or mip.iteration_number == 1):
            for random_i in range(int(parameters['number_of_random_starting_values'])):
                filename4 = create_nlp_filename( 'tight' + '_' + mip.id + '_random_initial_'+str(random_i) )
                file4 = open( 'gams_files/' + filename4, 'w' )
                file4.write( '$title "' + filename2.replace('.gms','') + '"\n')
                self.write_variable_definition_random_initial_value(file4, minlp, mip)
                self.write_constraints(file4, minlp, mip)
        ...
        if parameters['number_of_extra_nlps'] >= 1 and (mip.changed_binary_decission == 'yes' or mip.iteration_number == 1):
            for extra_i in range(ceil(int(parameters['number_of_extra_nlps'])/2)):
                filename4 = create_nlp_filename( 'tight' + '_' + mip.id + '_extra_down_initial_'+str(extra_i) )
                file4 = open( 'gams_files/' + filename4, 'w' )
                file4.write( '$title "' + filename2.replace('.gms','') + '"\n')
                self.write_variable_definition_extra_down_initial_value(file4, minlp, mip, extra_i, parameters['relative_stepzise_for_extra_nlps'])
                self.write_constraints(file4, minlp, mip)
            ...
            for extra_i in range(ceil(int(parameters['number_of_extra_nlps'])/2)):
                filename4 = create_nlp_filename( 'tight' + '_' + mip.id + '_extra_up_initial_'+str(extra_i) )
                file4 = open( 'gams_files/' + filename4, 'w' )
                file4.write( '$title "' + filename2.replace('.gms','') + '"\n')
                self.write_variable_definition_extra_up_initial_value(file4, minlp, mip, extra_i, parameters['relative_stepzise_for_extra_nlps'])
                self.write_constraints(file4, minlp, mip)
    ...
    def write_nlp_from_dc_solution(self, minlp, mip, dc_mip, parameters):
        ...
        if parameters['number_of_random_starting_values'] >= 1:
            for random_i in range(int(parameters['number_of_random_starting_values'])):
                filename4 = create_nlp_filename( dc_mip.id + '_random_initial_1_'+str(random_i) )
                ...
                file4 = open( 'gams_files/' + filename4, 'w' )
                file4.write( '$title "' + filename4.replace('.gms','') + '"\n')
                self.write_variable_definition_dc_random_initial_value(file4, minlp, dc_mip)
                self.write_constraints(file4, minlp, dc_mip)
                self.write_model_definition(file4, minlp, dc_mip)
                self.write_model_options(file4, minlp, dc_mip)
                file4.close()      
        ...
        if parameters['number_of_random_starting_values'] >= 1 and (dc_mip.changed_binary_decission == 'yes' or dc_mip.iteration_number == 1):
            for random_i in range(int(parameters['number_of_random_starting_values'])):
                filename4 = create_nlp_filename( dc_mip.id + '_random_initial_'+str(random_i) )
                ...
                file4 = open( 'gams_files/' + filename4, 'w' )
                file4.write( '$title "' + filename4.replace('.gms','') + '"\n')
                self.write_variable_definition_random_initial_value(file4, minlp, dc_mip)
                self.write_constraints(file4, minlp, dc_mip)
                self.write_model_definition(file4, minlp, dc_mip)
                self.write_model_options(file4, minlp, dc_mip)
                file4.close()
    ...
    def write_variable_definition_dc_random_initial_value(self, file, minlp, mip):
        file.write( '\n*===== SECTION: VARIABLE DEFINITION\n' )
        file.write( 'Free Variable objval ;\n' )
        for outer_key in minlp.variables.keys():
            for inner_key in minlp.variables[outer_key].keys():
