...
EPS = 1e-7
EPS_SUBSTITUTION = 0#1e-9
#TO DO: PDF-Datei mit Modell, delta-methode notation nach geißler, martin, schewe
#TO DO: erkenne doppelte substitutionen
...
class MipRelaxationBuilder(object):
    ...
    def build(self, minlp_reformulation, max_error_for_relaxation=inf, number_initial_segments=NUMBER_INITIAL_SEGMENTS, use_convex_quadratic=0, use_socp=0, number_outer_approx=0, use_nonconvex_qp=0, use_mccormick=0, use_domain_reduction=0, use_univariate=0, rest_time=inf):
        start_time = time()
        ...
        self.build_mip_relaxation(mip_relaxation, minlp_reformulation, max_error_for_relaxation, number_initial_segments, use_convex_quadratic, use_socp, number_outer_approx, use_nonconvex_qp, use_mccormick, use_domain_reduction, use_univariate, start_time, rest_time)    #build relaxation from reformulation
        if use_univariate == 1 and use_mccormick == 1 and use_nonconvex_qp==0 and time()-start_time<rest_time:
        ...
        if abs(time() - start_time) > rest_time:
            return 0
        else:
            return mip_relaxation
    
       
    def build_mip_relaxation(self, mip_relaxation, minlp_reformulation, max_error_for_relaxation, number_initial_segments, use_convex_quadratic, use_socp, number_outer_approx, use_nonconvex_qp, use_mccormick, use_domain_reduction, use_univariate, start_time, rest_time):
            ...
            if string_part[ len(string_part)-1 ] != SUBSTITUTION_SUFFIX and str(outer_key) != THERMAL_LIMIT_CONSTRAINT_NAME:    #constraints are linear
                ...
                if use_convex_quadratic == 0:
                    ...
                    if number_outer_approx>0:
                      ...
                      #Platzhalter für outer approximation
        ...
        if use_socp == 1 and time()-start_time<rest_time:
            self.build_socp_constraints(mip_relaxation, minlp_reformulation, use_univariate)
        if use_nonconvex_qp == 1 and time()-start_time<rest_time:
    ...
    def build_relaxation_from_constraint(self, constraint, new_outer_key, inner_key, relaxation_info_dic, new_variables_dic, new_constraints_dic, mip_relaxation, outer_key, max_error_for_relaxation, number_initial_segments, use_nonconvex_qp, use_mccormick, use_domain_reduction):
        ...
        if subst_var.lower_bound < subst_var.upper_bound:    #don't build relaxation for fixed values
            if constraint.type == BILINEAR_CONSTRAINT_TYPE_NAME:
                ...
                #To Do: if lower_bound == upper_bound
                ...
                #mip_relaxation.update_constraint_dic(constraint, outer_key_for_discrete_bilinear, '' , inner_key )
    ...
    def create_1d_relaxation_info(self, mip_relaxation, subst_var, var, constr_type, new_outer_key, inner_key, relaxation_info_dic, new_variables_dic, new_constraints_dic, max_error_for_relaxation, number_initial_segments, use_domain_reduction):
        ...
        if constr_type == 'quadratic':
            min_number_segments = ceil((var.upper_bound-var.lower_bound)/(2*sqrt(float(max_error_for_relaxation))))
            number_segments = max(min_number_segments, number_initial_segments)
            tri = Triangulation1d(tri_id, [var.lower_bound, var.upper_bound], number_segments)
            ...
            #if new_outer_key == 'trigonometric_relaxation' and inner_key == (1,2,'quadratic_2'):
            #    print('max_error',max_error_for_relaxation)
            #    print(tri.vertices)
            ...
            if max_error_for_relaxation > 0:
                while max(lower_error_on_segments) > float(max_error_for_relaxation) or max(upper_error_on_segments) > float(max_error_for_relaxation):
                    ...
                    #if constr_type == 'sin' or constr_type == 'cos' or constr_type == 'tan' or constr_type == 'quadratic':
                        #if tri.test_if_point_is_interior( 0, segment_number ) is True:
                            #tri.add_point( 0 )
                        #else:
    ...
    def create_1d_relaxation_vars(self, relaxation_info, new_variables_dic):
        ...
        delta_vars = {}
        for i in range(1, number_segments+1):    #nummerierung beginnt bei 1 und endet bei number_segments
        ...
        binary_vars = {}
        for i in range(1, number_segments):    #nummerierung begintt bei 1 und endet bei number_segments-1
    ...
    def create_1d_relaxation_y_value_constraint(self, relaxation_info, new_variables_dic, new_constraints_dic):
        ...
        #print(relaxation_info.var.id, relaxation_info.var.lower_bound, relaxation_info.var.upper_bound)
        #print(relaxation_info.id, relaxation_info.triangulation.vertices, relaxation_info.lower_errors, relaxation_info.upper_errors)
    ...
    def create_1d_domain_reduction_negativ(self, mip_relaxation, subst_var, var, constr_type, new_outer_key, inner_key, relaxation_info_dic, new_variables_dic, new_constraints_dic, max_error_for_relaxation, number_initial_segments, use_domain_reduction):
        ...
        #coupling of subst_var and sub_pos_var
    ...
    def create_2d_relaxation_x_value_constraints(self, relaxation_info,  new_variables_dic, new_constraints_dic):
        ...
        expression = Eq(var1.sympy_symbol , vertices[0][0][0]
                        + sum(sum( (vertices[i-1][j][0] - vertices[i-1][0][0]) * delta_vars[i,j].sympy_symbol
                                   for j in range(1,3) )
                              for i in range(1,number_segments+1) ) )
        '''
        expression = Eq(var1.sympy_symbol, vertices[0][0][0]
        ...
        '''
        expression = Eq(var2.sympy_symbol , vertices[0][0][1] + sum(sum( (vertices[i-1][j][1] - vertices[i-1][0][1]) * delta_vars[i,j].sympy_symbol
                                                                         for j in range(1,3) )
                                                                    for i in range(1,number_segments+1) ) )
        '''
    ...
    def create_2d_relaxation_error_bound_constraints(self, relaxation_info, new_variables_dic, new_constraints_dic):
        ...
        if max(upper_error_on_segments) > 0:
            ...
            #sum( binary_vars[i+1].sympy_symbol * (upper_error_on_segments[i+1] - upper_error_on_segments[i])
        ...
        if max(lower_error_on_segments) > 0:        
            expression_lower = lower_error_on_segments[0] + \
                               Add(*tuple((value.sympy_symbol * (lower_error_on_segments[key] - lower_error_on_segments[key-1])
                                           for (key, value) in binary_vars.items())))
            #sum( binary_vars[i+1].sympy_symbol * (lower_error_on_segments[i+1] - lower_error_on_segments[i])
            #                        for i in range(number_segments-1) )
                ...
                for key in minlp_reformulation.constraints['conic_quadratic_substitution'].keys():
                    if inner_key[0] == key[0] and inner_key[1] == key[1]:
                        ...
                        if len(str(key[2]).split('bilinear')) > 1:
                            ...
                            right_expression = right_expression + constr.involved_vars[1].sympy_symbol* constr.involved_vars[2].sympy_symbol #( 0.5*(info.var[0].sympy_symbol+info.var[1].sympy_symbol) )**2
                expression = Eq(left_expression , right_expression)#+ ( 0.5*(info.var[0].sympy_symbol-info.var[1].sympy_symbol) )**2 <= ( 0.5*(info.var[0].sympy_symbol+info.var[1].sympy_symbol) )**2#info.var[0].sympy_symbol* info.var[1].sympy_symbol
        ...
        for inner_key in  minlp_reformulation.constraints['conic_quadratic_reformulated'].keys():
            ...
            if number <= number_of_constr:
                ...
                if 'transmission_switch' in mip_relaxation.variables.keys():
                    ...
                    #voltage_squares_sum = mip_relaxation.variables['voltage_squares_sum'][inner_key]
                    ...
                    for key in minlp_reformulation.constraints['conic_quadratic_substitution'].keys():
                        if inner_key[0] == key[0] and inner_key[1] == key[1]:
                            ...
                            if len(str(key[2]).split('bilinear')) > 1:
                                ...
                                right_expression = right_expression + constr.involved_vars[1].sympy_symbol* constr.involved_vars[2].sympy_symbol #( 0.5*(info.var[0].sympy_symbol+info.var[1].sympy_symbol) )**2
                    expression = Le(left_expression , right_expression)#+ ( 0.5*(info.var[0].sympy_symbol-info.var[1].sympy_symbol) )**2 <= ( 0.5*(info.var[0].sympy_symbol+info.var[1].sympy_symbol) )**2#info.var[0].sympy_symbol* info.var[1].sympy_symbol
    ...
    def build_outer_approx(self, mip_relaxation, minlp_reformulation, number_outer_approx, new_dic):
        ...
        for inner_key in  minlp_reformulation.constraints[THERMAL_LIMIT_CONSTRAINT_NAME].keys():
            ...
            if max1**2 + max2**2 > limit:    #thermal limits are not redundant
                number = 1
                #print(number_outer_approx)
                for i in range(0, int(number_outer_approx)):
                    
                    angle = 0 + i*(2*pi)/number_outer_approx
                    if angle%(2*pi/4) != 0:#these constraints are fullfilled with bounds
    ...
    def build_mccormick_for_univariate_trigonometric(self, mip_relaxation, minlp_reformulation):
        ...
        for inner_key in minlp_reformulation.constraints['trigonometric_reformulated'].keys():
            ...
            if len( str(constr.expression).split('subst_tan_variable') ) > 1:
                ...
                if len(inner_key) == 3:
                    var2 = mip_relaxation.relaxation_infos['trigonometric_relaxation'][inner_key[0],inner_key[1],'tan'].subst_var                    
                else:
                    var2 = mip_relaxation.relaxation_infos['trigonometric_relaxation'][inner_key+('tan',)].subst_var
                ...
                lb1 = var1.lower_bound
                ub1 = var1.upper_bound
                lb2 = -var2.upper_bound
                ub2 = -var2.lower_bound
                #subst_var = relaxation_info.subst_var
...
def compute_error_on_all_1d_segments(constr_type, vertices):
    ...
    for i in range(number_segments):
        if abs(vertices[i] - vertices[i+1]) <= EPS:
            [lower_error_array[i], upper_error_array[i]] = [EPS_SUBSTITUTION, EPS_SUBSTITUTION]
        else:
...
def compute_cos_error_on_segment(point1, point2):   
    slope = (cos(point2)-cos(point1))/(point2-point1)    #slope
    intersec = (point2*cos(point1)-point1*cos(point2))/(point2-point1)    #intersec
    extreme1 = asin(-slope)    #extreme point
    if extreme1 >= 0:
...
def compute_tan_error_on_segment(point1, point2):
    ...
    slope = (tan(point2)-tan(point1))/(point2-point1)
    intersec = tan(point1)-slope*point1#(point2*tan(point1)-point1*tan(point2))/(point2-point1)
    ...
    print('error1', error1)
    print('error2', error2)
    print('lower_error', lower_error)
    print('upper_error', upper_error)
    '''
...
def create_2d_y_value_constraint_expression(constr_type, subst_var, vertices, delta_vars, error_var):
    ...
    if constr_type == BILINEAR_CONSTRAINT_TYPE_NAME:
        ...
        expression = Eq(subst_var.sympy_symbol , vertices[0][0][0]*vertices[0][0][1] +
                        sum(sum( (vertices[i-1][j][0]*vertices[i-1][j][1] - vertices[i-1][0][0]*vertices[i-1][0][1]) * delta_vars[i,j].sympy_symbol
                                 for j in range(1,3) ) for i in range(1,number_segments+1) ) + error_var.sympy_symbol)
