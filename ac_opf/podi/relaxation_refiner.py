...
from math import sqrt, tan, sin, cos, pi, inf, ceil
from time import time
from sympy import Array as array
...
class RelaxationRefiner(object):
    ...
    def refine_mip_relaxation(self, mip, parameters, minlp):
        ...
        #self.test_if_2d_cut_possible(mip, minlp, parameters)
        ...
        for outer_key in mip.relaxation_infos.keys():
            for inner_key in mip.relaxation_infos[outer_key].keys():
                ...
                if relaxation_info.relaxation_error_abs >= parameters['refinement_percentage_for_cuts'] * mip.max_relaxation_error_abs:
                    ...
                    #print(relaxation_info.constr_type)
                abs_relaxation_info = ''
    ...
    def refine_relaxation_variables(self, mip, outer_key, inner_key, info=''):
        ...
        #add new binary_variable if necessary  
        new = 0
        if inner_key in mip.variables[outer_key].keys():
        ...
        #update error_bounds
        if info.constr_type != 'abs' and DELTA_METHOD_ERROR_KEY in mip.variables[outer_key][inner_key].keys():
    ...
    def refine_x_value_2d(self, mip, outer_key, inner_key):
        ...
        variables1[0] = var1
        #print(relaxation_info.triangulation.number_of_segments, delta_vars)
    ...
    def refine_error_bound(self, mip, outer_key, inner_key):
        ...
        if max(upper_error_on_segments) > 0 and DELTA_METHOD_ERROR_BOUND_OVER_KEY in mip.constraints[outer_key][inner_key].keys():        
        ...
        if max(upper_error_on_segments) > 0 and DELTA_METHOD_ERROR_BOUND_OVER_KEY not in mip.constraints[outer_key][inner_key].keys():
        ...
        if max(lower_error_on_segments) > 0 and DELTA_METHOD_ERROR_BOUND_UNDER_KEY in mip.constraints[outer_key][inner_key].keys():        
        ...
        if max(lower_error_on_segments) > 0 and DELTA_METHOD_ERROR_BOUND_UNDER_KEY not in mip.constraints[outer_key][inner_key].keys():
    ...
    def tighten_vertices_with_new_bounds(self, mip):
        ...
        for outer_key in mip.relaxation_infos.keys():
            for inner_key in mip.relaxation_infos[outer_key].keys():
                ...
                if var.lower_bound > vertices[0] + NUMERIC_ERROR_SIMPLEX or var.upper_bound < vertices[-1] - NUMERIC_ERROR_SIMPLEX:
                    if info.constr_type == 'quadratic':
                        subst_low, subst_up = compute_quadr_subst_bound(var.lower_bound, var.upper_bound)
                    elif info.constr_type == 'sin':
                        subst_low, subst_up = compute_sin_subst_bound(var.lower_bound, var.upper_bound)
                    elif info.constr_type == 'cos':
                        subst_low, subst_up = compute_cos_subst_bound(var.lower_bound, var.upper_bound)
                    elif info.constr_type == 'tan':
                        subst_low, subst_up = compute_tan_subst_bound(var.lower_bound, var.upper_bound)
                    subst_var = info.subst_var
                    subst_var.set_variable_lower_bound(subst_low)
                    subst_var.set_variable_upper_bound(subst_up)
                    #print(info.triangulation.vertices)
                    info.triangulation.tight_vertices_with_new_bounds(var.lower_bound, var.upper_bound)
                    [lower_error_on_segments, upper_error_on_segments] = compute_error_on_all_1d_segments(info.constr_type, info.triangulation.vertices)            
                    info.set_lower_errors(lower_error_on_segments)
                    info.set_upper_errors(upper_error_on_segments)
                    self.refine_relaxation_variables(mip, outer_key, inner_key, info)
                    self.refine_relaxation_constraints(mip, outer_key, inner_key, info)                    
                    #print(info.triangulation.vertices)
                
        


    def refine_tight_mip(self, tight_mip, number_segments, error, time_start, time_limit, parameters, first_refinement=False):
        new_info = []
        for outer_key in tight_mip.relaxation_infos.keys():
            for inner_key in tight_mip.relaxation_infos[outer_key].keys():
                ...
                if time()-time_start < time_limit:
                    ...
                    for round in range(1, rounds_exponent+1):
   
                        for segment_number in range(1, info.triangulation.number_of_segments+1):
                            segment_number_after_refinement = 1 + 2 * (segment_number-1)
                            info.triangulation.longest_edge_bisection(segment_number_after_refinement)

                        if info.constr_type == BILINEAR_CONSTRAINT_TYPE_NAME:
                            [lower_error_on_segments, upper_error_on_segments] = compute_error_on_all_2d_segments(
                                info.constr_type, info.triangulation.vertices)
                        else:
                            [lower_error_on_segments, upper_error_on_segments] = compute_error_on_all_1d_segments(
                                info.constr_type, info.triangulation.vertices)
                        info.set_lower_errors(lower_error_on_segments)
                        info.set_upper_errors(upper_error_on_segments)
                ...
                if time()-time_start < time_limit:
                    if info.constr_type == 'quadratic':
                        length = info.triangulation.vertices[-1] - info.triangulation.vertices[0]
                        length = ceil(length/(2*sqrt(error)))
                        info.triangulation.enforce_minimum_segment_length( length )
                    else:
...
def compute_quadr_subst_bound(lb, ub):
    array = [ lb**2, ub**2 ]
    if lb<= 0 and 0<=ub:
        lower_bound = 0
    else:
...
def compute_sin_subst_bound(lb, ub):
    array = [ sin(lb) , sin(ub) ]
...
def compute_cos_subst_bound(lb, ub):
    array = [ cos(lb) , cos(ub) ]
    if lb <= -pi and -pi <= ub:
        lower_bound = -1
...
def compute_tan_subst_bound(lb, ub):  
